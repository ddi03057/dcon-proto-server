...
public class UserService implements UserServiceKeycloak {
    ...
    private static final String[] attributeArr = {UserOtherClaim.USER_NAME, UserOtherClaim.USER_LOCALE, UserOtherClaim.ERROR_CNT
                                                ...
                                                , UserOtherClaim.USER_EMAIL, UserOtherClaim.USE_YN, UserOtherClaim.COMPANY
                                                , UserOtherClaim.PHONE};
    private static final String[] ignoreProperties = {"userId", "email", "firstName", "lastName", "userFullName","company","phone"};
    ...
    public UserDTO selectCurrentUser() throws Exception {
        ...
        @SuppressWarnings("unchecked") //무점검 형변환 경고 제거
...
//            user.getAttributes().put(UserOtherClaim.SKIP_PWD_CHG_DT, Arrays.asList(String.valueOf(System.currentTimeMillis())));
    ...
    public ResultCode updateUser(UserChangeDTO useChg) {
        ...
        try {
            ...
            param.put(UserOtherClaim.COMPANY, useChg.getCompany());
            param.put(UserOtherClaim.PHONE, useChg.getPhone());
...
//                .grantType(OAuth2Constants.CLIENT_CREDENTIALS)
    ...
    public UserRepresentation setInsertUserRepresentation(UserRepresentation createUser, UserCreateDTO user) {
        ...
        attributes.put(UserOtherClaim.COMPANY, Arrays.asList(currentUserService.getCurrentUser().getCompany()));
        attributes.put(UserOtherClaim.PHONE, Arrays.asList(currentUserService.getCurrentUser().getPhone()));
